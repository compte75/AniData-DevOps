...
for fichier, description in fichiers.items():
    chemin = os.path.join(DATA_DIR, "archive", fichier)
...
anime = pd.read_csv(os.path.join(DATA_DIR, "archive", "anime.csv"))
ok(f"anime.csv : {anime.shape[0]:,} lignes × {anime.shape[1]} colonnes")

print("  Chargement de anime_with_synopsis.csv...")
synopsis = pd.read_csv(os.path.join(DATA_DIR, "archive", "anime_with_synopsis.csv"))
...
rating_sample = pd.read_csv(
    os.path.join(DATA_DIR, "archive" ,"rating_complete.csv"),
    nrows=500_000
)
# Compter le nombre total de lignes sans tout charger
print("  Comptage du nombre total de lignes (peut prendre 1-2 min)...")
total_ratings = sum(1 for _ in open(os.path.join(DATA_DIR,"archive","rating_complete.csv"))) - 1
